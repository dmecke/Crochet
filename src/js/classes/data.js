				...
				if (lines[i].trim().charAt(0) === '#') {
					...
					if (lines[i].includes('title:')) {
						...
						if (lines[i].includes(positionString)) {
							var xy = lines[i]
								.substr(positionString.length + 1)
								.trim()
								.split(',');
							obj.position = { x: Number(xy[0].trim()), y: Number(xy[1].trim()) };
						} else if (lines[i].includes(colorIDString)) {
							obj.colorID = Number(lines[i].substr(colorIDString.length).trim());
	...
	getSaveData: function(type) {
		...
		if (type == FILETYPE.JSON) {
			...
			for (let i = 0; i < content.length; i++) {
				...
				output += '__PrivCrochet_position: ' + content[i].position.x + ',' + content[i].position.y + '\n';
			...
			for (let i = 0; i < content.length; i++) {
				...
				output +=
					'\t\t<position x="' + content[i].position.x + '" y="' + content[i].position.y + '"></position>\n';
	...
	promptFileNameAndFormat: function(cb, suggestions = null) {
      ...
      <datalist class="form-control" id="select-file-name">    
        ${suggestions && suggestions.map(suggestion => `<option value="${suggestion}" />`).join('')}
			...
			onOpen: () => {
				if (data.editingName() !== 'NewFile') document.getElementById('swal-input1').value = data.editingName();
			},
			showCancelButton: true,
			...
			if (value && value !== '') {
				...
				const editingName = (data.editingName() || '').replace(/\.[^/.]+$/, '') + '.' + editingType;
	...
	trySaveCurrent: function() {
		if (data.lastStorageHost() === 'GIST') {
			...
			Swal.fire({
				...
				text:
					"Please export the file and re-open it to ensure changes are saved to your disk. If you don't, internal storage will be overwritten when you open another file and your changes will be lost.",
				icon: 'info',
			});
